...
def user(nickname):
    ...
    try:
        ...
        status = nickname == payload.get('id')
        user_info = db.users.find_one(
            {'nickname': nickname},
            {'_id': False}
        )
...
def sign_in():
    ...
    # Pesan debugging
    print(f"Received nickname: {nickname_receive}, Hashed password: {pw_hash}")

    result = db.users.find_one({'nickname': nickname_receive, 'password': pw_hash})
    ...
    if result is not None:
        ...
        return jsonify({'result': 'fail', 'msg': 'Username atau password salah'})
...
def update_profile():
    ...
    try:
        ...
        db.users.update_one({"nickname": username}, {"$set": new_doc})
        return jsonify({"result": "success", "msg": "Profile updated!"})
    
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError) as e:
        print(e)  # Log any exceptions for debugging purposes
        return jsonify({"result": "fail", "msg": "Token expired or decode error"})
...
def posting():
    ...
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=["HS256"])
        user_info = db.users.find_one({"nickname": payload["id"]})
...
def get_posts():
    ...
    try:
        ...
        nickname_receive = request.args.get("username_give")
        if nickname_receive == '':
            posts = list(db.posts.find({}).sort("date", -1).limit(20))
        else:
            posts = list(db.posts.find({"nickname": nickname_receive}).sort("date", -1).limit(20))
...
def update_like():
    ...
    try:
        ...
        user_info = db.users.find_one({"nickname": payload.get("id")})
